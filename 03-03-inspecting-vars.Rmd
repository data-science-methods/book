...
- A web search leads us to this City of Oakland page on police stop data:  <https://www.oaklandca.gov/resources/stop-data>
    ...
    - Our data runs through December 2017
...
- The easy solution is to count rows, using the tools we've already developed. 
    - As a twist, we'll use `mutate()` after `count()` to calculate shares, ie, what fraction of the total are in each group
...
- There are several directions we could take this analysis:  
    - Investigate outstanding questions about quality and reliability of the data
        ...
        - Fits with the *risk management* perspective of Boehm's spiral development model

    - Break down our question into sub-analyses
        - eg, the Oakland web site and report talk about policy changes; do we see changes by year in the data? 
        - Fits with Bogen and Woodward's *data-phenomena-theory* model

    - Apply more sophisticated analysis
        - eg, a regression model to control for age, gender, and other covariates
        - Fits with all three models of EDA
